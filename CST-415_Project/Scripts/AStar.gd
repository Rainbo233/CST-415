...
export (bool) var show_nav_cubes := false
...
func _ready() -> void:
	...
	var walkables = get_tree().get_nodes_in_group("Walkable")
	_add_points(walkables) 
	_connect_points()

func _add_points(walkables: Array):
	
	for walkable in walkables:
		
		var mesh = walkable.get_node("MeshInstance")
...
func _connect_points():
	for point in points:
		...
		var world_position = Vector3(pos_str[0], pos_str[1], pos_str[2])
		...
		for x in search_coords:
			for z in search_coords:
				...
				var potential_neighbor = world_to_astar(world_position + search_offset)
